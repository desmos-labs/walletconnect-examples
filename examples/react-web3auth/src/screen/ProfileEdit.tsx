...
export default function ProfileEdit(): JSX.Element {
  ...
  useEffect(() => {
    if (desmosClient !== undefined && signer !== undefined && signerStatus === SignerStatus.Connected) {
      (async () => {
        try {
          ...
          const profile = await desmosClient.getProfile(accounts[0].address);
          setProfileState({
            status: ProfileStatus.Fetched,
            profile,
          });
  ...
  }, [desmosClient, signer, signerStatus])

  const saveProfile = useCallback(async () => {
    if (signer !== undefined && desmosClient !== undefined && profile !== null) {
      try {
        ...
        await desmosClient.signAndBroadcast(creator, [{
  ...
  }, [signer, desmosClient, profile, profileState]);
